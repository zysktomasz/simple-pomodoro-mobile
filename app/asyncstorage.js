...
export const _saveSettingsFromStateToStorage = async (
  activityTime,
  breakTime,
  ...
  try {
    ...
    await AsyncStorage.setItem(
      "playSoundOnCountdownEnd",
      playSoundOnCountdownEnd.toString()
    );
    await AsyncStorage.setItem(
      "playSoundOnCountdownLastTicks",
      playSoundOnCountdownLastTicks.toString()
    );
...
export const _loadSettingsFromAsyncStorage = async () => {
  try {
    ...
    const playSoundOnCountdownEnd = await AsyncStorage.getItem(
      "playSoundOnCountdownEnd"
    );
    const playSoundOnCountdownLastTicks = await AsyncStorage.getItem(
      "playSoundOnCountdownLastTicks"
    );
    if (
      activityTime !== null ||
      breakTime !== null ||
      playSoundOnCountdownEnd !== null ||
